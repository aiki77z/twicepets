...
const PET_SPRITESHEETS = {
  navely: "./navely/hatch-run/final/spritesheet.webp?v=20260625-1",
  jeongvely: "./jeongvely/hatch-run/final/spritesheet.webp?v=20260625-1",
  movely: "./movely/hatch-run/final/spritesheet.webp?v=20260625-1",
  savely: "./savely/hatch-run/final/spritesheet.webp?v=20260625-1",
  jively: "./jively/hatch-run/final/spritesheet.webp?v=20260625-1",
  mively: "./mively/hatch-run/final/spritesheet.webp?v=20260625-1",
  davely: "./davely/hatch-run/final/spritesheet.webp?v=20260625-1",
  chaengvely: "./chaengvely/hatch-run/final/spritesheet.webp?v=20260625-1",
  tsuvely: "./tsuvely/hatch-run/final/spritesheet.webp?v=20260625-1"
};
...
let currentPetVisualBounds = { x: 0, y: 0, width: ATLAS.cellWidth, height: ATLAS.cellHeight };
...
function getTransparentBoundsFallback() {
  return { x: 0, y: 0, width: ATLAS.cellWidth, height: ATLAS.cellHeight };
}

function measureFrameBounds(ctx, x, y) {
  const data = ctx.getImageData(x, y, ATLAS.cellWidth, ATLAS.cellHeight).data;
  let minX = ATLAS.cellWidth;
  let minY = ATLAS.cellHeight;
  let maxX = -1;
  let maxY = -1;

  for (let py = 0; py < ATLAS.cellHeight; py += 1) {
    for (let px = 0; px < ATLAS.cellWidth; px += 1) {
      const alpha = data[(py * ATLAS.cellWidth + px) * 4 + 3];
      if (alpha < PET_LAYOUT.hitAlpha) continue;
...
function measurePetVisualBounds(image) {
  if (visualBoundsCache.has(image)) return visualBoundsCache.get(image);
  ...
  for (const action of ACTIONS) {
    for (let frame = 0; frame < action.frames; frame += 1) {
      const bounds = measureFrameBounds(ctx, frame * ATLAS.cellWidth, action.row * ATLAS.cellHeight);
      if (!bounds) continue;
      if (!union) {
        union = { ...bounds };
        continue;
  ...
  const measured = union || getTransparentBoundsFallback();
  visualBoundsCache.set(image, measured);
  return measured;
}

function applyPetVisualBounds(bounds) {
  const safeBounds = bounds?.width && bounds?.height ? bounds : getTransparentBoundsFallback();
  currentPetVisualBounds = safeBounds;
  petCanvas.width = Math.max(1, safeBounds.width * 2);
  petCanvas.height = Math.max(1, safeBounds.height * 2);

  const aspectRatio = safeBounds.width / safeBounds.height;
  document.documentElement.style.setProperty("--pet-aspect-ratio", String(aspectRatio || PET_LAYOUT.fallbackAspectRatio));

  if (HAS_DESKTOP_SHELL && window.desktopPetShell.setPetVisualBounds) {
    window.desktopPetShell.setPetVisualBounds({
...
function drawFrame(now, image) {
  const visualBounds = measurePetVisualBounds(image);
  if (
    visualBounds.width !== currentPetVisualBounds.width
    || visualBounds.height !== currentPetVisualBounds.height
    || visualBounds.x !== currentPetVisualBounds.x
    || visualBounds.y !== currentPetVisualBounds.y
  ) {
    applyPetVisualBounds(visualBounds);
  }

  const config = getActionConfig(state.action);
  const frame = frameIndexFor(now);
  const sx = frame * ATLAS.cellWidth + currentPetVisualBounds.x;
  const sy = config.row * ATLAS.cellHeight + currentPetVisualBounds.y;
  const maxScale = Math.min(
    petCanvas.width / currentPetVisualBounds.width,
    petCanvas.height / currentPetVisualBounds.height
  );
  const scale = maxScale >= 1 ? Math.floor(maxScale) || 1 : maxScale;
  const dw = Math.round(currentPetVisualBounds.width * scale);
  const dh = Math.round(currentPetVisualBounds.height * scale);
  ...
  petCtx.drawImage(image, sx, sy, currentPetVisualBounds.width, currentPetVisualBounds.height, dx, dy, dw, dh);
...
function shouldCapturePointer(event) {
  if (state.companionMode) return true;
  const target = event.target;
  if (state.bubbleEditorOpen && target.closest(".bubble-editor-dialog")) return true;
  if (state.nameEditorOpen && target.closest(".name-editor-dialog")) return true;
  if (!state.companionMode && (state.controlsOpen || state.controlsHoverOpen) && target.closest(".action-bar")) return true;
  if (target.closest("#petButton")) return isPetBodyPointerEvent(event);
  return false;
}

function updateWindowMousePassthrough(event) {
  if (!HAS_DESKTOP_SHELL) return;
  setPetInputTransparent(!shouldCapturePointer(event));
}
...
function updatePetDrag(event) {
  ...
  if (Math.abs(deltaX) > 6) {
    const nextDragAction = deltaX > 0 ? "running-right" : "running-left";
    if (dragState.dragAction !== nextDragAction) {
      dragState.dragAction = nextDragAction;
      activateAction(nextDragAction, { quiet: true });
    }
  }
...
function handleCompanionModeDoubleClick(event) {
  if (!state.companionMode) return;
  event.preventDefault();
  event.stopPropagation();
  setPetInputTransparent(false);
...
petButton.addEventListener("dblclick", handleCompanionModeDoubleClick);
petCluster.addEventListener("dblclick", handleCompanionModeDoubleClick);
...
document.addEventListener("pointermove", updateWindowMousePassthrough);
...
applyPetVisualBounds(currentPetVisualBounds);
setAction(DEFAULT_ACTION);
scheduleBubbleRotation();
window.requestAnimationFrame(animatePet);
