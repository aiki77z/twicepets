...
const APP_DISPLAY_NAME = "TWICE Pets";
const PET_SCALE_LIMITS = { min: 0.35, max: 1 };
const PET_BASE_WIDTH = 220;
const PET_FALLBACK_VISUAL_BOUNDS = { width: 192, height: 208 };
const PET_OPTIONS = [
  { id: "navely", label: "navely" },
  { id: "jeongvely", label: "jeongvely" },
  { id: "movely", label: "movely" },
  { id: "savely", label: "savely" },
  { id: "jively", label: "jively" },
  { id: "mively", label: "mively" },
  { id: "davely", label: "davely" },
  { id: "chaengvely", label: "chaengvely" },
  { id: "tsuvely", label: "tzuvely" }
];
const DEFAULT_PET_ID = "navely";
...
function normalizeVisualBounds(bounds) {
  ...
  if (!Number.isFinite(width) || !Number.isFinite(height) || width < 24 || height < 24) {
    return PET_FALLBACK_VISUAL_BOUNDS;
  }

  return {
    width: Math.min(width, PET_FALLBACK_VISUAL_BOUNDS.width),
    height: Math.min(height, PET_FALLBACK_VISUAL_BOUNDS.height)
  };
}
...
function getWindowSizeForPetScale(scale, options = {}) {
  ...
  const visualBounds = normalizeVisualBounds(options.visualBounds);
  const visualAspectRatio = visualBounds.width / visualBounds.height;
  const petWidth = PET_BASE_WIDTH * normalizedScale;
  const petHeight = petWidth / visualAspectRatio;
...
function getWindowSizeForPet(petId) {
  ...
  return getWindowSizeForPetScale(petState.petScale, {
    ...
    visualBounds: petState.visualBounds
  });
}
...
function createMacTrayIcon() {
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44">
      ...
      <text x="22" y="29" text-anchor="middle" font-family="-apple-system, BlinkMacSystemFont, Helvetica, Arial, sans-serif" font-size="21" font-weight="800" fill="#111111">T</text>
    </svg>
  `.trim();
...
function setPetVisualBounds(window, bounds) {
  if (!window || window.isDestroyed()) return PET_FALLBACK_VISUAL_BOUNDS;

  const petState = ensurePetWindowState(window.petId);
  const nextBounds = normalizeVisualBounds(bounds);
  const previousBounds = normalizeVisualBounds(petState.visualBounds);
  petState.visualBounds = nextBounds;

  if (nextBounds.width !== previousBounds.width || nextBounds.height !== previousBounds.height) {
    applyWindowSizeForPet(window, "bottom");
  }
...
ipcMain.handle("shell:set-pet-visual-bounds", (event, bounds) => {
  return { visualBounds: setPetVisualBounds(getEventWindow(event), bounds) };
});
